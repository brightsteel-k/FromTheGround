...
public class BatModel extends HierarchicalModel<Bat> {
   ...
   public void setupAnim(Bat bat, float x, float y, float scale, float yRot, float xRot) {
      if (bat.isResting()) {
         this.head.xRot = xRot * ((float)Math.PI / 180F);
         this.head.yRot = (float)Math.PI - yRot * ((float)Math.PI / 180F);
         ...
         this.body.xRot = ((float)Math.PI / 4F) + Mth.cos(scale * 0.1F) * 0.15F;
         this.body.yRot = 0.0F;
         this.rightWing.yRot = Mth.cos(scale * 74.48451F * ((float)Math.PI / 180F)) * (float)Math.PI * 0.25F;
         this.leftWing.yRot = -this.rightWing.yRot;
